...
from DLADAN_bert import DLADAN_Bert_C
...
from Dataloaders import DladanCriminalLoader
from law_processing_Criminal import get_law_graph_for_Criminal
from Formatters import SentenceFormatter_DC
...
def load_dataset(group_indexes):
    # train_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_small/train_chinese.pkl'
    # valid_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_small/valid_chinese.pkl'
    # test_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_small/test_chinese.pkl'
    
    # train_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_medium/train_chinese.pkl'
    # valid_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_medium/valid_chinese.pkl'
    # test_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_medium/test_chinese.pkl'
    
    train_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_large/train_chinese.pkl'
    valid_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_large/valid_chinese.pkl'
    test_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/processed_dataset/Criminal/Criminal_large/test_chinese.pkl'

    train_Dataset = DladanCriminalLoader(data_path=train_path, group_indexes=group_indexes, mode='train')
    valid_Dataset = DladanCriminalLoader(data_path=valid_path, group_indexes=group_indexes, mode='valid')
    test_Dataset = DladanCriminalLoader(data_path=test_path, group_indexes=group_indexes, mode='test')
    
    Formatter = SentenceFormatter_DC(mode='train')
    def collate_fn(data):
    ...
    valid_dataset = DataLoader(dataset=valid_Dataset, batch_size=256,
                               num_workers=1, drop_last=False,
                               shuffle=False, collate_fn=collate_fn,
                               # sampler=RandomSampler(Dataset)
                               )
    
    test_dataset = DataLoader(dataset=test_Dataset, batch_size=256,
                               num_workers=1, drop_last=False,
                               shuffle=False, collate_fn=collate_fn,
                               # sampler=RandomSampler(Dataset)
                               )
...
def train_loop(model: DLADAN_Bert_C, train_dataset, valid_dataset, test_dataset, 
               epoches, scheduler, model_save_path, 
               warming_up_epoches, momentum_steps=1, gradient_accumulation_steps=1):
    scaler = GradScaler()
    best_f1 = 0
    ...
    for epoch_num in range(epoches):
        ...
        for step, data in enumerate(tqdm_obj):
            ...
            labels = [data['accu_label'], data['group_label']]
            ...
            if (step + 1) % gradient_accumulation_steps == 0 or (step + 1) % len(train_dataset) == 0:
...
if __name__ == "__main__":
    ...
    # 1. load dataset
    config_file= '../Config/DLADAN_Bert_Criminal.json'
    with open(config_file) as f:
        config = json.load(f, object_hook=lambda d: SimpleNamespace(**d))
    warming_up_epoch = 1
    
    law_inputs, group_list, graph_membership_law, graph_membership_charge, law_adj_matrix = \
        get_law_graph_for_Criminal(threshold=0.30, 
                                   pretrained_bert_fold=config.train.pretrain_model_path_1,
                                   max_length=config.train.law_sentence_len)
    group_indexes_charge = list(zip(*graph_membership_charge))[1]
    print(len(group_indexes_charge))
    group_indexes_law = list(zip(*graph_membership_law))[1]
    group_num = len(group_list)
    train_dataset, valid_dataset, test_dataset = load_dataset(group_indexes_charge)
    
     # 2. define model 
    model:DLADAN_Bert_C = \
        DLADAN_Bert_C(config=config, group_num=group_num, group_indexes=group_indexes_law,
    ...
    # 3. Training loop
    model_save_path = '/home/nxu/Ladan_tnnls/Bert_Ladan/model_save/Criminal-L/DLADAN_chinese_full.hdf5'
    train_loop(model=model, train_dataset=train_dataset, valid_dataset=valid_dataset, test_dataset=test_dataset,
               ...
               warming_up_epoches=3, momentum_steps=1)
